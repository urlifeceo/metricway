...
function saveMapping(projectToken: string, productId: string) {
...
<template>
  <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-base font-semibold text-gray-200">Продукты</h2>
      <span class="text-xs text-gray-500">Классификация для расчёта MRR / ARR / Churn</span>
    </div>

    <div v-if="!store.unknownProducts.length && !store.productMappings.length" class="text-sm text-gray-500 py-2">
      Нет данных о покупках
    </div>

    <div v-else class="space-y-4">
      <div v-if="store.unknownProducts.length">
        <div class="text-xs text-yellow-400 uppercase tracking-wider mb-2">
        ...
        <div class="space-y-2">
          <div
            ...
            class="flex flex-wrap items-center justify-between gap-3 bg-gray-800/60 border border-gray-700 rounded-lg px-4 py-2.5"
          >
            <div class="min-w-0">
              <div class="text-sm text-gray-200 font-medium truncate">{{ product.product_id }}</div>
              <div class="text-xs text-gray-500">{{ product.payments_count }} платежей · последний {{ product.last_amount }}</div>
            </div>
      ...
      <div v-if="store.productMappings.length">
        <div class="text-xs text-gray-500 uppercase tracking-wider mb-2">Классифицированные</div>
        <div class="flex flex-wrap gap-2">
          <div
            v-for="mapping in store.productMappings"
            :key="mapping.product_id"
            class="flex items-center gap-2 bg-gray-800 text-xs px-2.5 py-1 rounded-md border border-gray-700"
          >
            <span class="text-gray-300">{{ mapping.product_id }}</span>
            <span
              class="px-1.5 py-0.5 rounded font-medium"
              :class="mapping.billing_type === 'monthly' ? 'bg-blue-500/20 text-blue-300' : mapping.billing_type === 'yearly' ? 'bg-indigo-500/20 text-indigo-300' : 'bg-gray-600/40 text-gray-300'"
            >
